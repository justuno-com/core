...
final class Tag {
	...
	/** @used-by self::render() */
	private function content():string {return juc($this, function() {
		$c = ju_trim(ju_cc_n($this->_content), "\n"); /** @var string $c */
		return $this->tagIs('pre', 'code') || !ju_contains($c, "\n") ? $c : "\n" . ju_tab_multiline($c) . "\n";
	});}

	/** @used-by self::render() */
	private function openTagWithAttributesAsText():string {return ju_cc_s(
		$this->_tag
		,$this->_multiline ? "\n" : null
		,call_user_func(

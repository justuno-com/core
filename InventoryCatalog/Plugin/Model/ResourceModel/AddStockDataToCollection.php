<?php
namespace Justuno\Core\InventoryCatalog\Plugin\Model\ResourceModel;
use Closure as F;
...
/**
 * 2020-11-23
 * @see \Justuno\Core\CatalogInventory\Plugin\Model\ResourceModel\Stock\Status
...
final class AddStockDataToCollection {
	...
	function aroundExecute(Sb $sb, F $f, C $c, $skipAbsent, $stockId) {
		if (!self::$PRESERVE_ABSENT_F && !$c->getFlag(self::PRESERVE_ABSENT)) {
			...
			if ($stockId === ju_default_stock_provider()->getId()) {
				$s->joinLeft(
					['stock_status_index' => ju_table('cataloginventory_stock_status')],
				...
				$s->join(
					['product' => ju_table('catalog_product_entity')],
					sprintf('product.entity_id = %s.entity_id', C::MAIN_TABLE_ALIAS),
					[]
				);
				$s->joinLeft(
					['stock_status_index' => ju_stock_index_table_name_resolver()->execute($stockId)],
	 ...
	 */
	const PRESERVE_ABSENT = 'justuno_preserve_absent';

	/**
	 * 2020-11-23
	 * @used-by aroundExecute()
	 * @used-by ju_pc_preserve_absent_f()
	 * @used-by \Justuno\Core\CatalogInventory\Plugin\Model\ResourceModel\Stock\Status::beforeAddStockDataToCollection()
	 * @var bool
	 */

...
final class Client {
	...
	function captureException(Th $t, array $data):void {
		...
		do {
			...
			$dataI = [
				...
				,'value' => $this->serializer->serialize(ju_xts($t))
			];
			$trace = $t->getTrace();
	...
	private function capture($data, array $trace = []):string {
		$data += [
			...
			,'message' => substr(jua($data, 'message', ''), 0, self::MESSAGE_LIMIT)
			,'platform' => 'php'
			,'project' => $this->_projectId
			,'sdk' => ['name' => 'mage2.pro', 'version' => ju_core_version()]
			,'site' => jua($_SERVER, 'SERVER_NAME')
			,'tags' => []
		...
		/**
		 * 2017-01-10
		 * Намеренно использую здесь + вместо @see df_extend(),
		...
		$data['extra']  = Extra::fits($extraS = ju_dump($extra)) ? ['_dump' => $extraS] : Extra::adjust($extra);
		$data = ju_clean($data);
	...
	private function send_http(string $url, string $data, array $headers = []):void {
		...
		try {
			curl_setopt($c, CURLOPT_HTTPHEADER, ju_map_k(
	...
	private function get_curl_options():array {
		$r = [
			...
			# 2023-07-15
			# 1) "The Sentry's TLS certificate (`cacert.pem`) has expired": https://github.com/justuno-com/core/issues/391

...
final class Error extends \Justuno\Core\Qa\Failure {
	/**
	 * 2015-04-04
	 * 2022-12-14 PHP supports "{$this::t()}" (unlike "{$this->t()}") since PHP 5.3: https://3v4l.org/HBFTd
	 * @override
	 * @see \Justuno\Core\Qa\Failure::mai`2n()
	 ...
	 * @override
	 * @see \Justuno\Core\Qa\Failure::preface()
	 * @used-by \Justuno\Core\Qa\Failure::report()
	 * @used-by self::report()
	 */
	protected function preface():string {return ju_kv(
		ju_context() + ['File' => ju_path_relative($this::info('file')), 'Line' => $this::info('line')]
	);}
	...
	private function log():void {
		...
		if (!$inProcess) {
			$inProcess = true;
			try {
				ju_report('mage2.pro/{date}--{time}.log', $this->report());
				$inProcess = false;
			}
	 ...
	 * @used-by vendor/justuno.com/core/registration.php
	 */
	 ...
	 * @return string|int
	 */
	private static function info(string $k) {return jua(error_get_last(), $k);}
	 ...
	 * @used-by self::main()
	 */
	private static function msg():string {return ju_adjust_paths_in_message(self::info('message'));}
	...
	private static function type(bool $asString = false) {
		$r = self::info('type'); /** @var int|string $r */
		return !$asString ? $r : jua(self::map(), $r);
	}
